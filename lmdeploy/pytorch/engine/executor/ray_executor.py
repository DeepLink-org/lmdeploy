...
def get_ascend_device_rank_mapping(master_addr: str, workers: list, dp: int):
    ...
    if dp > 1:
        ...
        rank_mapping = {int(os.environ['RANK']): rank_mapping[int(os.environ['RANK'])]}
...
class RayExecutor(ExecutorBase):
    ...
    def _init_distributed_environment_by_device(self, device_str: str):
        ...
        if device_str == 'cuda':
            ...
            print(f"driver_ip: {driver_ip}, rank_mapping: {rank_mapping}, worker_ips: {worker_ips}, envs: {envs}, self.workers: {self.workers}", flush=True)
            if self.dp <= 1:
                ray.get([worker.set_device.remote(rank_mapping[idx]) for idx, worker in enumerate(self.workers)])
            else:
                ray.get([self.workers[0].set_device.remote(device_id) for device_id in rank_mapping.values()])

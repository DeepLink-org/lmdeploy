...
def apply_rotary_pos_emb(q_states: Tensor, k_states: Tensor, cached_cos: Tensor, cached_sin: Tensor, position_ids: Tensor, position_ids_1d: Tensor, q_embed=None, k_embed=None):
...
#def apply_rotary_pos_emb(q: Tensor, k: Tensor, cos: Tensor, sin: Tensor, position_ids: Tensor, position_ids_1d: Tensor, q_embed=None, k_embed=None):
#    bs, head, dim = q.shape
...
#        q[i] = torch.cat((outq0, outq1), dim=-1)
#
#        k0 = k[i:i+1, :, :dim//2]
#        k1 = k[i:i+1, :, dim//2:]
#        outk0 = k0 * cos[i, :dim//2] - k1 * sin[i, :dim//2]
#        outk1 = k0 * sin[i, :dim//2] + k1 * cos[i, :dim//2]
#        k[i] = torch.cat((outk0, outk1), dim=-1)
#    return q.view(bs, head, dim).contiguous(), k.view(bs, numKeyValueHeads, dim).contiguous()
...
#    return q.view(bs, head, dim), k.view(bs, numKeyValueHeads, dim)

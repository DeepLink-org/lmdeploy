...
class PatchedDeepseekV2AttentionAscend(nn.Module):
    ...
    def _contiguous_batching_forward_impl(
        ...
        def __rotary_emb_fn(q_pe, k_pe, out_q_pe, out_k_pe):
            """rope."""
            from ..kernels.default.apply_rotary_pos_emb import apply_rotary_pos_emb
            if not hasattr(context, '_cos'):
                cos, sin = self.rotary_emb(q_pe, seq_len=max_kv_seq_length)
                context._cos = cos
                context._sin = sin
...
class PatchedDeepseekV2MoEAscend(nn.Module):

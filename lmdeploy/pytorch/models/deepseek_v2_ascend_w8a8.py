...
class DeepseekV2Attention(nn.Module):
    ...
    def _kv_proj(self, hidden_states, nope_size: int):
        ...
        q_len = hidden_states.size(1)
        key_states = self.kv_a_proj_with_mqa(hidden_states[0, :, None]).view(q_len, 1, -1)
        kv_c, k_pe = torch.split(key_states, [self.kv_lora_rank, self.qk_rope_head_dim], dim=-1)
        # kv_a_layernorm
        kv_c_normed = self.kv_a_layernorm(kv_c).view(q_len, -1)
    ...
    def _qkv_proj(self, hidden_states: torch.Tensor, num_heads: int):
        ...
        q_nope, q_pe = self._q_proj(hidden_states, num_heads, nope_size, pe_size)
        kv_c_normed, k_pe = self._kv_proj(hidden_states, self.kv_lora_rank)

        return q_nope, q_pe, kv_c_normed, k_pe
    ...
    def forward(
        ...
        # qkv_proj
        q_nope, q_pe, kv_c_normed, k_pe = self._qkv_proj(hidden_states, num_heads=num_heads)

        cos, sin = rotary_pos_emb
        q_pe_shape, k_pe_shape = q_pe.shape, k_pe.shape
        b, h_q, d = q_pe.shape
        q_pe = q_pe.view(b, h_q, d // 2, 2).transpose(3, 2).reshape(b, h_q, d)
        b, h_k, d = k_pe.shape
        k_pe = k_pe.view(b, h_k, d // 2, 2).transpose(3, 2).reshape(b, h_k, d)
        import torch_npu
        self.cos_sin_cache = torch.cat((cos, sin), dim=-1)
        q_pe = q_pe.contiguous().view(q_pe.shape[0], -1)
        k_pe = k_pe.contiguous().view(q_pe.shape[0], -1)
        ...
        q_pe = q_pe.view(q_pe_shape)
        k_pe = k_pe.view(k_pe_shape)


        key_states_cached = torch.cat((kv_c_normed.unsqueeze(1), k_pe), dim=-1)
        if not attn_metadata.is_decoding:
            kv_b_proj_weight = torch.cat((self.kc.weight, self.vc.weight.transpose(1, 2).contiguous()), dim=1).view(-1, self.kv_lora_rank)
            kv = torch.nn.functional.linear(kv_c_normed, kv_b_proj_weight).view(q_len, num_heads, -1)
            k_nope, value_states = kv.split([self.qk_nope_head_dim, self.v_head_dim], dim=-1)
            k_pe = k_pe.repeat(1, num_heads, 1)
            key_states = torch.cat((k_nope, k_pe), dim=-1)
        else:
            q_nope_t = torch.transpose(q_nope, 0, 1)
            q_nope_out = torch.bmm(q_nope_t, self.kc.weight)
            q_nope = torch.transpose(q_nope_out, 0, 1)
            value_states = kv_c_normed
            key_states = key_states_cached
        query_states = torch.cat((q_nope, q_pe), dim=-1)
        from ..kernels.dlinfer import fill_kv_cache
        past_key_value = fill_kv_cache(key_states_cached,
                                              value_states,
                                              past_key_value[0],
                                              past_key_value[0][..., :nope_size],
                                              attn_metadata.kv_start_indices,
                                              None, None)
        ...
        attn_output = self.o_proj(attn_output.reshape(q_len, -1))
...
class DeepseekV2MoE(nn.Module):
    ...
    def __init__(self, config: Any, layer_idx, dtype: torch.dtype = None, device: torch.device = None):
        ...
        quantization_config = {
            "quant_method": "dlinfer_ascend_w8a8_dynamic",
            "quant_dtype": "torch.int8",
        }
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        out_states = out_states.reshape(sequence_length, -1)
...
class DeepseekV2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.mlp(hidden_states).unsqueeze(0)
...
class DeepseekV2Model(nn.Module):
    ...
    def forward(
        ...
        """forward."""
        attn_metadata.position_ids = position_ids.to(torch.int64)
